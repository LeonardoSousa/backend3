...
class RemessaController extends Controller
{
    ...
    function index()
    {
        return ModelsRemessa::orderBy('id', 'DESC')->get();
    }
    ...
    function store()
    {
        ...
        // $boletos = Boleto::with('cliente')
        //     ->whereHas('cliente', function (Builder $query) use ($vencimento) {
        //         $query->where('venc', '=', $vencimento);
        ...
        Boleto::query()->whereIn('id', $boletos->pluck('id'))
            ->update(['status' => 'GERADO', 'remessa_id' => $remessaDb->id]);
    ...
    public function download($id)
    {
        return response()->streamDownload(function() use($id)  {
            echo Storage::disk('local')->get("remessas/remessa-{$id}.txt");
        }, "remessa-{$id}.txt", ['Content-Type' => 'text/plain']);
